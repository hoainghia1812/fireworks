...
// When true, automatic shell launching is suppressed (used during countdown)
...
fscreen.addEventListener('fullscreenchange', () => {
  ...
  // Ensure layout updates when fullscreen mode changes so canvases resize properly.
  // Some browsers don't emit a window resize on fullscreen change, so call handleResize if available.
  try {
    if (typeof handleResize === 'function') {
      // small timeout to let fullscreen layout settle
      setTimeout(handleResize, 60);
    }
  } catch (e) {
    // ignore if handleResize not defined yet
  }
});
...
const store = {
  ...
  state: {
    ...
    paused: true,
    // enable sound by default so auto-launch attempts to play sounds when allowed
    soundEnabled: true,
    menuOpen: false,
    openHelpTopic: null,
...
const palmShell = (size=1) => {
  // Palm shells produce strong horizontal streamers. Only enable them when
  // a text is being displayed (_bgInterval active). Otherwise fall back to a
  // gentler default shell to avoid horizontal spray.
  if (!_bgInterval) return crysanthemumShell(size);
...
const fallingLeavesShell = (size=1) => {
  // Falling leaves produce drifting horizontal particles. Only enable during text display.
  if (!_bgInterval) return crysanthemumShell(size);
  return {
    shellSize: size,
    color: INVISIBLE,
...
const willowShell = (size=1) => {
  // Willows have long downward trails; restrict to text display.
  if (!_bgInterval) return crysanthemumShell(size);
  return {
    shellSize: size,
    spreadSize: 300 + size * 100,
...
const horsetailShell = (size=1) => {
  // Horsetail creates heavy trailing effects - only allow during text display.
  if (!_bgInterval) return crysanthemumShell(size);
  const color = randomColor();
  return {
    shellSize: size,
...
function randomShell(size) {
  ...
  // Normal operation
  // Avoid launching 'horizontal-spray' heavy shells except while a text is being shown.
  // These shells include types like Horse Tail, Palm, Willow, Falling Leaves which create strong horizontal streaks.
  const RESTRICTED_SHELLS = ['Horse Tail', 'Palm', 'Willow', 'Falling Leaves'];
  let name = randomShellName();
  // If not currently showing a text (no background interval), avoid restricted shells by retrying a few times.
  if (!_bgInterval) {
    let attempts = 5;
    while (attempts-- > 0 && RESTRICTED_SHELLS.includes(name)) {
      name = randomShellName();
    }
  }
...
function randomFastShell() {
  ...
  if (isRandom) {
    // Avoid selecting heavy horizontal shells unless text is visible
    const RESTRICTED_SHELLS = ['Horse Tail', 'Palm', 'Willow', 'Falling Leaves'];
    let attempts = 6;
    while (fastShellBlacklist.includes(shellName) || (! _bgInterval && RESTRICTED_SHELLS.includes(shellName))) {
...
function launchShellFromConfig(event) {
  // If countdown is active, suppress manual launches
...
function startSequence() {
  // If countdown active, suppress automatic shell launching. Return a short delay to retry.
  if (countdownActive) {
    return 1000;
  }
  if (isFirstSeq) {
...
if (IS_HEADER) {
  ...
  setTimeout(() => {
    ...
    .then(
      () => {
        // Attempt to enable audio for auto-launch. Browsers may still require a user gesture.
        try {
        ...
        // If audio is suspended by the browser, provide a best-effort automatic retry
        // to resume the AudioContext. If that fails, show an overlay prompting the user
        // to tap to enable sound.
        function showEnableSoundOverlay() {
          if (document.getElementById('enable-sound-overlay')) return;
          const div = document.createElement('div');
          div.id = 'enable-sound-overlay';
          div.innerHTML = 'Chạm để bật âm thanh<br><small>Tap to enable sound</small>';
          const onFirstTouch = function onFirstTouch(e) {
            try {
              soundManager.resumeAll();
              store.setState({ soundEnabled: true });
            } catch (err) {}
            div.removeEventListener('pointerdown', onFirstTouch);
            if (div.parentNode) div.parentNode.removeChild(div);
          };
          div.addEventListener('pointerdown', onFirstTouch, { once: true });
          document.body.appendChild(div);
        }

        function hideEnableSoundOverlay() {
          const el = document.getElementById('enable-sound-overlay');
          if (el && el.parentNode) el.parentNode.removeChild(el);
        }

        let _autoAudioRetryTimer = null;
        function tryAutoEnableAudio(attempts = 8) {
          try {
            soundManager.resumeAll();
          } catch (err) {}
          ...
          try {
            ...
            if (ctx && ctx.state === 'running') {
              ...
              try { soundManager.playSound('burst', 0.6); } catch (e) {}
              return;
            }
          } catch (err) {}
          ...
          if (_autoAudioRetryTimer) clearTimeout(_autoAudioRetryTimer);
          _autoAudioRetryTimer = setTimeout(() => {
            tryAutoEnableAudio(attempts - 1);
          }, 1200);
        }

        function onVisibilityOrFocus() {
          tryAutoEnableAudio(6);
        }
        ...
        document.addEventListener('pointerdown', resumeOnFirstInteraction, { passive: true });

        // Start best-effort autoplay attempts immediately. If the browser allows it, audio will start
        // without any user action; otherwise, the overlay will be shown after retries.
        tryAutoEnableAudio(8);
...
// Create a burst that forms text made of particles.
function addTextBurst(text, centerX, centerY, opts = {}) {
  const color = opts.color || COLOR.White;
  const scale = opts.scale || 1.0;
  const particleStep = opts.step || 6; // sampling step in pixels
  // Default life longer so text fades slowly (in ms)
  const life = opts.life || 12000;

  // Wrapping: determine max width to use for text before sampling
  const maxWidth = opts.maxWidth || Math.min(stageW * 0.8, 1200);

  // Prepare an offscreen canvas to draw wrapped text
  const canvas = document.createElement('canvas');
  const ctx = canvas.getContext('2d');

  // Start with base font size and decrease if a single word is too wide
  let fontSize = Math.floor(80 * scale);
  ctx.textBaseline = 'middle';
  ctx.textAlign = 'left';

  // Function to set font and measure
  const setFont = (fs) => {
    ctx.font = `bold ${fs}px Russo One, Arial`;
  };
  setFont(fontSize);

  // Word-wrap algorithm: split by spaces and build lines
  const wrapTextToLines = (txt) => {
  ...
  // Ensure no single word is wider than maxWidth by shrinking fontSize if necessary
  const words = text.split(/\s+/);
  let maxWordWidth = 0;
  words.forEach(w => maxWordWidth = Math.max(maxWordWidth, ctx.measureText(w).width));
  while (maxWordWidth > maxWidth && fontSize > 12) {
    fontSize -= 2;
    setFont(fontSize);
    maxWordWidth = 0;
    words.forEach(w => maxWordWidth = Math.max(maxWordWidth, ctx.measureText(w).width));
  ...
  // Draw lines centered inside canvas
  setFont(fontSize);
  ...
  // For each pixel sample, spawn a star that will travel from center->target
  for (let y = 0; y < canvas.height; y += particleStep) {
    for (let x = 0; x < canvas.width; x += particleStep) {
      ...
      if (alpha > 128) {
        ...
        const relX = (x - canvas.width / 2) * (scale * 0.9);
        const relY = (y - canvas.height / 2) * (scale * 0.9);
        const targetX = centerX + relX;
        const targetY = centerY + relY;
        ...
        // speed such that particle reaches near its target in ~ 0.5 - 1.2s
        const travelTime = 0.6 + Math.random() * 0.6; // seconds
        const speed = Math.min(4, dist / (travelTime * 60));
        ...
        // Make them relatively stable (less air drag), so they sit into place
        star.heavy = false;
        star.sparkFreq = 0;
        star.prevX = centerX;
        star.prevY = centerY;
        // Give small damping so they slow near target
        star.arriveX = targetX;
        star.arriveY = targetY;
        star.arrive = true;
      }
    }
  }

  // Play a small burst sound and flash
  BurstFlash.add(centerX, centerY, Math.max(60, Math.min(160, textW / 2)));
  try { soundManager.playSound('burst'); } catch (e) {}
}

// Hook into update loop to gently pull 'arrive' stars toward their arriveX/Y
const _orig_update = typeof update === 'function' ? update : null;
function update_with_arrive(frameTime, lag) {
  if (typeof update === 'function') {
    // run the original update behavior first
    _orig_update && _orig_update(frameTime, lag);
  }

  // gently nudge arriving stars toward their target
  const speedFactor = 1; // keep consistent with update's speed multiplier
  COLOR_CODES_W_INVIS.forEach(color => {
    const stars = Star.active[color];
    for (let i = stars.length - 1; i >= 0; i--) {
      const s = stars[i];
      if (s && s.arrive) {
        const dx = s.arriveX - s.x;
        const dy = s.arriveY - s.y;
  // move a fraction towards the arrival point (slower so text eases in)
  s.x += dx * 0.06 * speedFactor * lag;
  s.y += dy * 0.06 * speedFactor * lag;
        // damp velocity so it settles
        s.speedX *= 0.96;
        s.speedY *= 0.96;
      }
    }
...
// Replace ticker handler to include arrival behavior
mainStage.removeEventListener && mainStage.removeEventListener('ticker', update);
mainStage.addEventListener('ticker', update_with_arrive);

// Sequence of messages and associated firework color themes.
let _textLaunchTimer = null;
let _textSequenceIndex = 0;
let _bgInterval = null;
let _normalFireInterval = null; // interval used during the 1-minute free fireworks
let _finalShown = false;
const TEXT_FIREWORK_SEQUENCE = [
  { text: 'HAPPY NEW YEAR 2026', textLife: 7000, fireworkColors: [COLOR.Gold, COLOR.Red], fireworkLife: 7000 },
  { text: 'MÃ ĐÁO THÀNH CÔNG', textLife: 7000, fireworkColors: [COLOR.Red], fireworkLife: 7000 },
  { text: 'CHÚC MỪNG NĂM MỚI! AN KHANG – THỊNH VƯỢNG – VẠN SỰ NHƯ Ý', textLife: 7000, fireworkColors: [COLOR.Gold, COLOR.White], fireworkLife: 7000 },
  { text: 'NĂM MỚI ĐẾN, CHÚC BẠN LUÔN MẠNH KHỎE VÀ NHIỀU NIỀM VUI!', textLife: 7000, fireworkColors: [COLOR.Blue, COLOR.Purple], fireworkLife: 7000 },
  { text: 'NĂM MỚI THÀNH CÔNG RỰC RỠ, TIỀN VÀO NHƯ NƯỚC', textLife: 7000, fireworkColors: [COLOR.Gold, COLOR.Green], fireworkLife: 7000 },
  { text: 'MỌI CHUYỆN THUẬN LỢI', textLife: 7000, fireworkColors: [COLOR.Purple, COLOR.White], fireworkLife: 7000 },
  { text: 'CHÚC BẠN NĂM 2026 THẬT RỰC RỠ, THẬT THÀNH CÔNG', textLife: 7000, fireworkColors: [COLOR.Red, COLOR.Gold], fireworkLife: 7000 }
];

function _startNormalFireworks(durationMs = 60000) {
  // Launch regular random shells periodically for durationMs, then show final message.
  if (_normalFireInterval) return;
  const start = Date.now();
  _normalFireInterval = setInterval(() => {
    if (!store.state.config.autoLaunch) return;
    const sizeInfo = getRandomShellSize();
    const shell = new Shell(crysanthemumShell(Math.max(1, shellSizeSelector())));
    shell.launch(sizeInfo.x, sizeInfo.height);
    if (Date.now() - start >= durationMs) {
      clearInterval(_normalFireInterval);
      _normalFireInterval = null;
      // show final message after a short delay to let last bursts finish
      setTimeout(() => showFinalThankYou(), 800);
    }
  }, 900);
}
...
function showFinalThankYou() {
  ...
  _finalShown = true;
  // Stop any background intervals/timers
  stopTextLaunches();
  if (_normalFireInterval) { clearInterval(_normalFireInterval); _normalFireInterval = null; }

  const centerX = stageW / 2;
  const centerY = stageH * 0.45;
  // larger, centered thank-you text
  addTextBurst('CẢM ƠN', centerX, centerY, { color: COLOR.White, scale: 1.4, step: 5, life: 8000 });
}

function startTextLaunches() {
  if (_textLaunchTimer) return;

  _textSequenceIndex = 0;

  const runNext = () => {
    // If autoLaunch disabled, poll and retry later
    if (!store.state.config.autoLaunch) {
      _textLaunchTimer = setTimeout(runNext, 3000);
      return;
    }

    // If we've shown all messages, start 1 minute of normal fireworks then show final thank-you
    if (_textSequenceIndex >= TEXT_FIREWORK_SEQUENCE.length) {
      _textLaunchTimer = null;
      _startNormalFireworks(60 * 1000);
      return;
    }

    const item = TEXT_FIREWORK_SEQUENCE[_textSequenceIndex];
    const centerX = stageW / 2;
    const centerY = stageH * 0.35;

    // Show text for item.textLife
    addTextBurst(item.text, centerX, centerY, { color: COLOR.White, scale: 1.0, step: 6, life: item.textLife });

    // Start background fireworks while text is visible
    if (_bgInterval) {
    ...
    _bgInterval = setInterval(() => {
      if (!store.state.config.autoLaunch) return;
      const offsetX = (Math.random() - 0.5) * (stageW * 0.25);
      const launchX = Math.min(Math.max((centerX + offsetX - 60) / (stageW - 120), 0.05), 0.95);
      const shellOpts = crysanthemumShell(Math.max(1, shellSizeSelector() - 1));
      const shell = new Shell(shellOpts);
      shell.launch(launchX, 0.55 + Math.random() * 0.15);
    }, 1400);

    // After text life elapses, launch a themed shell, then prepare next item
    _textLaunchTimer = setTimeout(() => {
      const shellOpts = crysanthemumShell(shellSizeSelector());
      const shell = new Shell(shellOpts);
      const launchXNorm = (centerX - 60) / (stageW - 120);
      const clampedX = Math.min(Math.max(launchXNorm, 0.05), 0.95);
      shell.launch(clampedX, 0.6);

      if (_bgInterval) { clearInterval(_bgInterval); _bgInterval = null; }

      _textLaunchTimer = setTimeout(() => {
        _textLaunchTimer = null;
        _textSequenceIndex += 1;
        // small gap before next text
        _textLaunchTimer = setTimeout(runNext, 600);
      }, item.fireworkLife);
    }, item.textLife);
  };

  // start immediately after short delay so init can complete
  _textLaunchTimer = setTimeout(runNext, 1200);
}

function stopTextLaunches() {
  if (_textLaunchTimer) {
    clearTimeout(_textLaunchTimer);
    _textLaunchTimer = null;
  }
  if (_bgInterval) {
    clearInterval(_bgInterval);
...
// Real-time countdown overlay (counts down to next New Year local time)
function startRealCountdown(onComplete) {
  ...
  Object.assign(div.style, {
    ...
    fontFamily: 'Russo One, Arial, sans-serif', flexDirection: 'column'
  });

  const timeBox = document.createElement('div');
  timeBox.style.fontSize = '88px';
  timeBox.style.fontWeight = '800';
  timeBox.style.textAlign = 'center';
  timeBox.style.letterSpacing = '2px';
  timeBox.textContent = '--:--:--';

  const label = document.createElement('div');
  label.style.fontSize = '18px';
  label.style.marginTop = '12px';
  label.textContent = 'Thời gian đến Năm Mới';

  const hint = document.createElement('div');
  hint.style.fontSize = '13px';
  hint.style.marginTop = '8px';
  hint.style.opacity = '0.85';
  hint.textContent = 'Đang đếm ngược...';
  ...
  // mark countdown active to suppress automatic shell launches
  countdownActive = true;

  // Countdown decorative effects
  let effectsInterval = null;
  function startCountdownEffects() {
    // spawn gentle decorative stars around center
    effectsInterval = setInterval(() => {
      const cx = stageW / 2;
      const cy = stageH * 0.35;
      const angle = Math.random() * Math.PI * 2;
      const r = (Math.random() * 0.35 + 0.05) * Math.min(stageW, stageH) * 0.5;
      const x = cx + Math.cos(angle) * r;
      const y = cy + Math.sin(angle) * r * 0.6;
      const star = Star.add(x, y, COLOR.Gold, Math.random() * Math.PI * 2, 0.2 + Math.random() * 0.8, 2400 + Math.random() * 1600);
      star.visible = true;
      star.sparkFreq = 0;
      // subtle burst flash occasionally
      if (Math.random() < 0.12) BurstFlash.add(x, y, 30 + Math.random() * 60);
    }, 220);
  }

  function stopCountdownEffects() {
  ...
  // Compute next New Year target (local midnight of next year)
  ...
  function tick() {
    ...
    timeBox.textContent = formatTime(diff);
    // small pulsing effect on timeBox
    const scale = 1 + Math.sin(Date.now() / 400) * 0.02;
    timeBox.style.transform = `scale(${scale})`;
  ...
  function cleanup() {
    ...
    // allow auto-launches again
    countdownActive = false;
  }

  // start
  startCountdownEffects();
  tick();
}

// Start text launches after init if autoLaunch is enabled, but show a 10s countdown first
const _orig_init = typeof init === 'function' ? init : null;
function init_with_text() {
  // mark countdown active before init to ensure no auto-launch happens during initialization
  countdownActive = true;
  _orig_init && _orig_init();
  // start a real-time countdown to next New Year; when complete, start the sequence
  startRealCountdown(() => {
    startTextLaunches();
  });
}
if (typeof init === 'function') {
  // replace init reference used earlier
  init = init_with_text;
}